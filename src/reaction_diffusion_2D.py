    ...
    def creer_video(self, fichier, nb_etapes, couleur):
        ...
        if couleur:
            video = cv.VideoWriter(
                "rendus/"+fichier, cv.VideoWriter_fourcc(*"mp4v"), fps=10.0, frameSize=self.forme)
        else:
            video = cv.VideoWriter("rendus/"+fichier, cv.VideoWriter_fourcc(
                *"mp4v"), fps=10.0, frameSize=self.forme, isColor=False)
...
if modele == "gs":
    ...
    def GS_Rb(a, b):
        return a*b*b-(f+k)*b
    Sim = Simulation2D(Da, Db, GS_Ra, GS_Rb, initialisation_GS,
                       dt, (largeur, longueur), etapes, symetrie)
    ...
    def FN_Rb(a, b):
        return (a - b) * beta
    Sim = Simulation2D(Da, Db, FN_Ra, FN_Rb, initialisation_aleatoire,
                       dt, (largeur, longueur), etapes, symetrie)
