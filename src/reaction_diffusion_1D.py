...
class Simulation1D():
    ...
    def simulation(self):
        ...
        """
        print("Tapez la croix rouge pour sortir de la simulation.")
        self.initialisation()
        fig, ax = plt.subplots()
...
if modele == "gs":
    ...
    def GS_Rb(a, b):
        return a*b*b-(f+k)*b
    Sim = Simulation1D(Da, Db, GS_Ra, GS_Rb, initialisation_GS,
                       dt, longueur, etapes, (-0.5, 1.5))
    ...
    def FN_Rb(a, b):
        return (a - b) * beta
    Sim = Simulation1D(Da, Db, FN_Ra, FN_Rb, perturbation,
                       dt, longueur, etapes, (-1, 1))
